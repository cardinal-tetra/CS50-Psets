    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (*(first_space + 1) != '/')
            {
                ...
                // Store the request method and check
            char* method = malloc(first_space - line + 1);
            strncpy(method, line, first_space - line);
            ...
            if (*(second_space + 1) != 'H')
            {
                ...
                // Store the request target and check
            char* request_target = malloc(second_space - first_space + 1);
            strncpy(request_target, first_space + 1, second_space - first_space -1);
            ...
            if (strchr(request_target, '.') == NULL)
            {
                ...
                // Store the HTTP version and check
            char* CRLF = strstr(line, "\r\n");
            char* remainder = malloc(CRLF - second_space + 2);
            strcpy(remainder, second_space + 1);
            ...
            if (strstr(remainder, "HTTP/1.1") == NULL)
            {
                error(505);
                continue;
            }    
            ...
            char* q_mark = strchr(request_target, '?');
            char query[strlen(request_target)];
            char ab_path[strlen(request_target)];
            
            if (q_mark == NULL)
            {
                strcpy(ab_path, request_target);
                query[0] = '\0';
            }
            else if (*(q_mark + 1) == '\0')
            {
                strncpy(ab_path, request_target, q_mark - request_target);
                ab_path[q_mark - request_target] = '\0';
                query[0] = '\0'; 
            }
            else 
            {
                ...
                strcpy(query, q_mark + 1);
                
                // check for errors
                if (strchr(ab_path, '?') != NULL)
                {
                    error(400);
                    continue;
                }
            ...
            char path[strlen(root) + strlen(ab_path) + 1];
            strcpy(path, root);
            strcat(path, ab_path);
            ...
            char* ex_begin = strchr(ab_path, '.');
            strcpy(extension, ex_begin + 1);
